...
**What landed (committed 2026-05-28):**
- **Step 2 — `sf_bitmap` (8 b) → `sf_index` (2 b).** Originally shipped as
  `11320bd` using `self.allowed_data_sfs` (the gateway's static home-leaf
  config) — broke cross-leaf forwards in s15 (DM 87%→53%, channels 89%→54%)
  because gateways switch active layer per-flight and the per-leaf SF lists
  are disjoint (s15: L1=`[7,9]`, L2=`[6,10]`, L3=`[10,11]`). Reverted as
  `4f05370`. **Re-applied as `c20585b` with `active_allowed_data_sfs(self)`**
  at pack_rts / parse_rts (mirrors the existing `active_allowed_sf_bitmap`
  helper), plus `tx_layer_rec.allowed_data_sfs` for cross-layer forwards
  through `issue_send`. 4-seed sweep on **both s18 (single-layer) and s15
  (three-layer with disjoint lists)** is byte-for-byte identical to HEAD.
  Frees 6 bits in byte 6 for future packing.
- **Lesson recorded as
  `feedback_wire_format_multileaf_sweep`:** any pack_*/parse_* change MUST
  sweep s15 + s17 + s18, not just s18. Single-layer scenarios hide
  cross-leaf semantic bugs because the gateway-layer-switch path doesn't
  fire there.
